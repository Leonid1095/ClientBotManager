...
# Импортируем данные для разделов
...
async def handle_portfolio(message: types.Message):
	for case in PORTFOLIO:
		kb = InlineKeyboardMarkup().add(InlineKeyboardButton("Посмотреть кейс", callback_data=f"case_{case['id']}"))
...
async def show_case_details(callback_query: types.CallbackQuery):
	...
	if case:
		await callback_query.message.answer(f"<b>{case['title']}</b>\n{case['details']}", parse_mode="HTML")
	await callback_query.answer()

# Статус заказа (заглушка)
...
async def handle_about(message: types.Message):
	await message.answer("Я — разработчик Telegram-ботов с опытом 3+ года. Более 50 реализованных проектов для бизнеса и частных лиц.\n\nПортфолио и отзывы доступны в соответствующих разделах.")
...
# - Стартовая точка для FSM и других функций
...
async def process_hosting(message: types.Message, state: FSMContext):
	...
	if bonus > 0:
		...
		summary = (
			...
			f"Файл: {'Есть' if data['file'] else 'Нет'}\n"
...
async def process_confirm(message: types.Message, state: FSMContext):
	...
	await message.answer(f"Спасибо! Ваша заявка принята. Ваш номер заказа: {order_id}\nВы можете проверить статус через меню 'Статус заказа'.")
...
# Простая in-memory база для рефералов и бонусов (можно заменить на Google Sheets)
REFERRALS = {}
BONUSES = {}
